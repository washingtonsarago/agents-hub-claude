#!/usr/bin/env node
// validate-artifacts — content + manifest validator for agents/, commands/,
...
const AUTONOMOUS_DIR = path.join(REPO_ROOT, 'autonomous');
...
// `autonomous` is optional: a hub with no autonomous agent has no such key, and
// manifests written before the category existed must still validate.
if (manifest.autonomous !== undefined && !Array.isArray(manifest.autonomous)) {
  fail('manifest', `manifest.autonomous must be an array when present (got ${typeof manifest.autonomous})`);
}
if (manifest.autonomous === undefined && fs.existsSync(AUTONOMOUS_DIR) && listFiles(AUTONOMOUS_DIR).length) {
  fail('manifest', 'autonomous/ has specs but manifest.autonomous is missing (run scripts/regen-manifest.js)');
}
...
const manifestNames = {
  ...
  autonomous: new Set((manifest.autonomous || []).map(it => it.name)),
...
// ============================================================
// 4. Each autonomous spec is well-formed and matches manifest
// ============================================================
// These describe agents that run unattended on a schedule, so the invariants
// here are safety rails, not style: a bad cron runs at the wrong hour forever,
// and an unreviewed `mode: write` takes irreversible action with nobody watching.
step('4. autonomous/');
const autonomousFiles = listFiles(AUTONOMOUS_DIR);
for (const file of autonomousFiles) {
  const stem = file.replace(/\.md$/, '');
  const who = `autonomous/${file}`;
  const fm = parseFrontmatter(fs.readFileSync(path.join(AUTONOMOUS_DIR, file), 'utf8'), who);
  if (!fm) continue;

  if (!fm.name) fail(who, 'frontmatter missing required field: name');
  else if (fm.name !== stem) fail(who, `frontmatter name "${fm.name}" must match filename stem "${stem}"`);

  if (!fm.description) fail(who, 'frontmatter missing required field: description');
  else {
    const dec = decodeMaybeQuoted(fm.description);
    if (!dec.ok) fail(who, `description: ${dec.error}`);
  }

  if (!fm.schedule) fail(who, 'frontmatter missing required field: schedule (5-field cron, UTC)');
  else if (fm.schedule.trim().split(/\s+/).length !== 5) {
    fail(who, `schedule must be a 5-field cron expression (got "${fm.schedule}")`);
  }

  if (!fm.repo) fail(who, 'frontmatter missing required field: repo');
  else if (!/^[\w.-]+\/[\w.-]+$/.test(fm.repo)) {
    fail(who, `repo must be in org/repo form (got "${fm.repo}")`);
  }

  if (!fm.model) fail(who, 'frontmatter missing required field: model');
  else if (!['opus', 'sonnet', 'haiku'].includes(fm.model)) {
    fail(who, `model must be opus|sonnet|haiku (got "${fm.model}")`);
  }

  if (!fm.mode) fail(who, 'frontmatter missing required field: mode (read-only|write)');
  else if (!['read-only', 'write'].includes(fm.mode)) {
    fail(who, `mode must be read-only|write (got "${fm.mode}")`);
  } else if (fm.mode === 'write' && !fm.approved_by) {
    // The friction is the point: authorizing an unattended agent to take
    // irreversible action must carry a name.
    fail(who, 'mode: write requires an explicit `approved_by: <nome>` field');
  }

  if (!fm.tools) fail(who, 'frontmatter missing required field: tools');

  if (fm.routine_id && !/^trig_[A-Za-z0-9]+$/.test(fm.routine_id)) {
    fail(who, `routine_id must look like trig_... when present (got "${fm.routine_id}")`);
  }

  if (!manifestNames.autonomous.has(stem)) {
    fail(who, 'not registered in manifest.autonomous (run scripts/regen-manifest.js)');
  }
  tick(who);
}
for (const it of manifest.autonomous || []) {
  if (!autonomousFiles.includes(`${it.name}.md`)) {
    fail('manifest.autonomous', `${it.name}: file ${it.path} does not exist`);
  }
}

// ============================================================
// 5. Each skill has SKILL.md with frontmatter + manifest cross-ref
// ============================================================
step('5. skills/');
...
// ============================================================
// 6. sha256 in manifest matches every file (single-file categories, skills multi)
// ============================================================
step('6. sha256 integrity');
for (const key of ['agents', 'commands', 'autonomous']) {
  for (const it of manifest[key] || []) {
